...
실제 확인된 9개 단지 분양공고 사례(의정부역 센트럴 아이파크, 영통역 우미린, 김포 풍무
레이크에듀시티, 번영로 롯데캐슬, 더폴 우정, 더샵 송도그란테르(아파트), 힐스테이트 안양
펠루스, 더폴 울산신정, 더샵 송도그란테르 G5-3블록(오피스텔))의 헤더 6패턴/코드 5형식/
층구분 8형식/동호 6형식/날짜 5형식/에어컨 4형식과, 더폴 울산신정·송도그란테르
오피스텔의 실제 원문을 `test_cases_d.js`~`test_cases_f.js`로 회귀 고정했다
(`e2e_real_test.js`, `e2e_v65_test.js`, `e2e_v65_songdo_test.js`로 브라우저 UI까지 검증).

가격 섹션 컬럼맵은 이제 **헤더 라벨을 그대로 읽는 방식이 아니라, 데이터 행 자체의
"대지비+건축비+(부가세)=합계" 내적 일관성으로 오프셋과 컬럼 개수를 스스로 보정**하는
방식이 기본이다(`detectPriceColumnStructure`). 헤더가 "1차 2차 1차(10%) 2차(10%)
4차(10%) 4차(10%)..."처럼 회차 라벨이 밀리거나 중복되어 깨져 있어도 영향을 받지 않는다.
중도금 날짜도 "중도금N차" 라벨에 의존하지 않고, 라벨 없이 나열된 연속 날짜 구간을
찾는 방식(`detectMidDatesGeneric`)을 함께 쓴다. 표가 행 경계에서 개행/공백 없이 그대로
붙어 복사되는 경우("189,000,0009층", "...84OB")도 콤마 자릿수 일관성으로 교정한다
(`fixGluedNumbers`). 다만 아래는 여전히 제한적으로만 지원된다.
