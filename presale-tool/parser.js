...
(function (root) {
  ...
  function splitDocumentSections(fullText) {
    ...
    if (areaStart >= 0 && priceStart >= 0) {
      ...
      // 두 번째(별도) "■ 공급금액 표"가 문서 뒤쪽에, balcony/option 앵커보다도 늦게 또
      // 있는 경우가 있다(실사례: 전남광주통합특별시 북구 임동 아파트 - "최상층 특화(펜트)
      // 세대"만 따로 다루는 두 번째 표가 "■ 공통 유의사항" 섹션 안에 흩어져 있다). 이
      // 두 번째 표는 다른 타입(200B/207F/209C/211E)의 가격을 메인 표와 중복해서 다시
      // 보여주기도 해서, 통째로 이어붙이면 그 타입들의 세대수가 두 배로 잘못 집계된다.
      // 그래서 표 전체를 붙이는 대신, 기존 복구를 다 거치고도 여전히 가격 행이 하나도
      // 없는 코드만 targeted로 찾는다. 이런 코드(214A/214D)는 긴 관리코드가 줄바꿈
      // 지점에서 쪼개진 채("214.618\n3A ...") 남아 있어, 줄 시작에 층 표기가 와야 하는
      // repairUndercountedPriceRows의 재개 지점 판별로도 못 찾는다(그 줄이 "3A 107동
      // 1호 1 49층 1 ..."처럼 코드 조각으로 시작). area 섹션에서 이미 정확히 읽어낸 그
      // 코드의 전용면적(exclusive_area)으로 온전한 긴 코드 문자열을 직접 재구성해
      // (예: "214A" + 전용면적 214.6183 -> "214.6183A"), 두 번째 표 구간을 재결합한
      // 토큰에서 그 문자열을 정확히 찾아 그 지점부터 일정 구간만 잘라 붙인다(그 코드가
      // 실제로 다시 등장하지 않으면 아무 것도 붙이지 않으므로 다른 문서에는 영향이 없다).
      ...
      var extraPriceStart = (stillMissingCodes.length && priceEndIdx >= 0) ? findEarliestMatch(text, SECTION_ANCHORS.price, priceEndIdx) : -1;
      if (extraPriceStart >= 0) {
        var widerPriceBound = nextMajorStart >= 0 && nextMajorStart > extraPriceStart ? nextMajorStart : text.length;
        ...
        stillMissingCodes.forEach(function (code) {
          ...
          if (!a || !isFinite(a.exclusive_area)) return;
          var letterSuffix = (code.match(/[A-Za-z]+\d*$/) || [''])[0];
          if (!letterSuffix) return;
          var expectedLong = a.exclusive_area.toFixed(4) + letterSuffix;
          // "■ 주택형 표시 안내" 같은 코드 매핑 표에도 같은 긴 코드가(줄바꿈 없이 온전한
          // 형태로) 먼저 나올 수 있어, 첫 번째 occurrence가 아니라 마지막 occurrence를
          // 쓴다 - 실제 가격 행은 항상 이 문서에서 더 뒤쪽에(줄바꿈으로 쪼개진 채) 있다.
          var idx = extraTokens.lastIndexOf(expectedLong);
          if (idx === -1) return;
          snippets.push(extraTokens.slice(idx, Math.min(extraTokens.length, idx + 20)).join(' '));
        });
        if (snippets.length) {
