 ...
 *
 * Contract:
 *   parseAreaSection(text, isOfficetel?)         -> [{code, exclusive_area, supply_area, supply_units}]
...
(function (root) {
  ...
  // 위와 같은 이유이지만 관리코드 자체가 소수가 아니라 순수 정수인 문서도 있다(실사례:
  // 힐스테이트 송파더그리드 오피스텔 - "60 28 60.3737 25.0137 ..."처럼 약식표기가 그냥
  // "60"이고, 그 세대수(28) 바로 뒤에 전용면적(60.3737)이 이어짐). 레터 접미사형(34A/34B
  // 등)과 이런 순수 숫자형(36/48/52/60 등)이 한 표 안에 섞여 있으면, 순수 숫자형은
  // SHORT_CODE_RE(문자 접미사 필수)/LONG_CODE_RE(소수 필수)/BARE_LONG_CODE_RE(소수 필수)
  // 어디에도 안 걸려 boundaries 자체가 하나도 안 잡히고, 그 표 구간이 통째로 직전 레터
  // 접미사 코드의 꼬리로 잘못 흡수된다(가격 섹션에서도 같은 코드로 잘못 흡수돼 세대수가
  // 부풀려짐). "숫자 하나(2~3자리) 뒤 몇 토큰 안에 그 숫자와 정수부가 똑같은 소수(전용
  // 면적)가 있는지"로 자기참조 검증하는 방식은 findSelfConsistentBareCodeBoundaries와
  // 동일하되, 후보 토큰 자체가 소수가 아니라 순수 정수라는 점만 다르다.
  // 순수 정수(2~3자리)는 세대수·기타 수치로도 흔해 findSelfConsistentBareCodeBoundaries의
  // 느슨한(뒤 5토큰 이내 아무 곳) 검증 방식을 그대로 쓰면 우연한 오탐 위험이 크다(실측:
  // 세대수 "13"이 다른 행의 "13.x" 대지지분 값과 우연히 부합해 코드로 오인됨). 그래서
  // "코드 바로 뒤(정확히 2칸 뒤)에 그 코드와 정수부가 같은 전용면적이 오고, 그 사이(1칸
  // 뒤)에는 세대수 모양의 정수가 온다"는 실제 행 구조(코드·세대수·전용면적 순서)와 정확히
  // 일치할 때만 인정해 오탐 가능성을 크게 줄인다.
  function findSelfConsistentBareIntCodeBoundaries(tokens) {
    var out = [];
    for (var i = 0; i < tokens.length; i++) {
      if (!/^\d{2,3}$/.test(tokens[i])) continue;
      if (!/^\d{1,4}$/.test(tokens[i + 1] || '')) continue;
      var m = /^(\d{2,3})\.\d+$/.exec(tokens[i + 2] || '');
      if (m && Number(m[1]) === Number(tokens[i])) out.push(i);
    }
    return out;
  }
      ...
      for (var j = i + 1; j < floats.length; j++) {
        ...
        for (var k = 0; k < floats.length; k++) {
          ...
          if (Math.abs(floats[k].v - sum) <= 0.02 && (!best || floats[k].v < best.c)) {
            best = { a: floats[i].v, b: floats[j].v, c: floats[k].v, ai: i, bi: j, ci: k };
          }
        }
      }
    }
  ...
  // 오피스텔 문서 중에는 "전용+공용=소계(공급면적)"에서 한 단계 더 나아가
  // "소계+기타공용면적(주차장면적 포함)=계약면적"까지 누적되는 표가 있다(실사례: 힐스테이트
  // 송파더그리드 오피스텔 - 공급호실 34.9930 15.2004 50.1934 30.2670 80.4604 10.4468 순서로
  // 전용/공용/소계/기타공용/계약면적/대지지분이 이어진다). 이 경우 findAreaTriple이 찾는
  // 가장 안쪽 합(소계=50.1934)은 오피스텔 문서가 실제로 표기하는 "계약면적"(80.4604)이
  // 아니다. 표 헤더에 "계약면적"이라는 단어가 실제로 등장할 때만(문서 구조 자체가 증거)
  // 안쪽 triple을 이미 사용한 세 값(전용/공용/소계)과는 다른 새 두 값(기타공용/계약면적)으로
  // 한 단계 더 확장을 시도한다 - 대지지분처럼 우연히 다른 합과 맞아떨어지는 값까지 이미 쓴
  // 인덱스로 제한해 오탐을 막는다.
  function findOuterAreaExtension(floats, inner) {
    var used = {};
    used[inner.ai] = used[inner.bi] = used[inner.ci] = true;
    for (var j = 0; j < floats.length; j++) {
      if (used[j]) continue;
      var sum = floats[inner.ci].v + floats[j].v;
      for (var k = 0; k < floats.length; k++) {
        if (k === inner.ci || used[k] || k === j) continue;
        if (Math.abs(floats[k].v - sum) <= 0.02) return floats[k].v;
      }
    }
    return null;
  }
  ...
  function extractAreaFromTokens(toks, preferContractArea) {
    var floats = [];
    var ints = [];
    ...
    // 세그먼트 경계 자체가 findSelfConsistentBareIntCodeBoundaries로 이미 "정수부가 같은
    // 소수(전용면적)가 근처에 있다"는 자기참조 검증을 거쳐 확정된 것이므로, 여기서도 같은
    // 기준으로 첫 토큰을 코드로 채택한다. 이 확인을 안 하면 바로 아래 "float 바로 앞/뒤
    // 정수" 폴백이 코드(36)가 아니라 세대수(13, float 바로 앞 토큰)를 코드로 잘못 집는다
    ...
    if (triple) {
      ...
      if (preferContractArea) {
        var outer = findOuterAreaExtension(floats, triple);
        if (outer != null) supply_area = outer;
      }
    } else {
  ...
  // "◯◯◯.####A 주택형은 103A1, 103A2, 103A3로 구분되며"처럼, 여러 하위코드가 하나의
  // 관리코드 아래 "통합청약"으로 묶이는 문서가 있다(실사례: 103A1/103A2/103A3,
  // 103B1/103B2/103B3, 112A1/112A2/112A3, 128A1/128A2/128A3 - 청약 시 통합 접수되어
  // 개별 선택이 불가함을 안내하는 각주). 이런 문서의 공급대상표는 PDF 재구성 순서상 이전
  // 행의 마지막 숫자값(최하층우선배정세대수 등)이 공백 없이 바로 다음 하위코드 앞에
  // 들러붙는 경우가 있다(실측: "195 5103A2" - "5"가 앞 행 값, "103A2"가 진짜 코드. 이걸
  // 못 떼면 "5103A2"라는 존재하지 않는 코드로 잘못 인식되어 진짜 103A2가 통째로 유실된다).
  // 이 각주가 명시적으로 나열하는 진짜 하위코드 목록을 근거로, 그 코드 바로 앞에 숫자가
  // 공백 없이 붙은 자리만 공백을 끼워넣는다(각주에 실제로 선언된 코드만 대상으로 삼아 다른
  // 문맥의 우연한 숫자+코드 조합까지 건드리지 않도록 한다).
  function fixGluedSubCodesFromFootnote(text) {
    var footnoteRe = /주택형은\s*((?:\d{2,4}[A-Za-z]{1,3}\d{0,2}\s*,\s*)+\d{2,4}[A-Za-z]{1,3}\d{0,2})\s*(?:로|으로)\s*구분/g;
    var declared = {};
    var m;
    while ((m = footnoteRe.exec(text))) {
      m[1].split(',').forEach(function (c) {
        var code = c.trim();
        if (code) declared[code] = true;
      });
    }
    var codes = Object.keys(declared);
    if (!codes.length) return text;
    var out = text;
    codes.forEach(function (code) {
      var re = new RegExp('(\\d)(' + code.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ')\\b', 'g');
      out = out.replace(re, '$1 $2');
    });
    return out;
  }

  function parseAreaSection(text, isOfficetel) {
    text = fixGluedNumbers(String(text || '').replace(/㎡/g, ' '));
    text = fixGluedSubCodesFromFootnote(text);
    // "전용+공용=소계(공급면적), 소계+기타공용면적=계약면적"처럼 두 단계로 누적되는 표는
    // 아파트/오피스텔 모두에 나온다(실사례: 의왕역 SK VIEW 아파트도 이 구조를 그대로 씀) -
    // "계약면적"이라는 단어의 등장 여부만으로는 어느 단계를 원하는지 구분할 수 없다. 오피스텔은
    // 관행상 계약면적(바깥쪽, 기타공용면적까지 포함)을, 아파트는 공급면적(안쪽, 소계)을 쓰므로
    // isOfficetel(사용자가 폼에서 고른 단지 종류)로만 판단한다.
    var preferContractArea = !!isOfficetel && /계약면적/.test(text);
    ...
    // 아래 중복 경계 판별(withFloatNext)이 이 경계들도 함께 걸러낼 수 있도록, 그 판별보다
    // 먼저 합쳐둔다.
    var bareBoundaries = findSelfConsistentBareCodeBoundaries(tokens)
      .concat(findSelfConsistentBareIntCodeBoundaries(tokens));
    ...
    // 있어 "84A 84B 84C ..."가 한 번 더 경계로 잡힌다. 힐스테이트 송파더그리드 오피스텔은
    // "■ 거주자 우선분양" 표가 같은 코드를 세대수 없이 다른 숫자(우선공급 배정 등)와 함께 또
    // 나열한다). 매핑/부가 표의 코드 나열은 뒤에 다시 코드(나 무관한 숫자)가 이어지지만, 실제
    // 데이터 행은 코드 바로 뒤(또는 세대수 하나를 사이에 두고)에 소수 면적값이 온다는 구조적
    // 차이로 실제 데이터 행만 남긴다(해당 코드에 그런 occurrence가 하나도 없으면 판단 근거가
    // 없으므로 원래대로 전부 남겨 안전하게 후퇴한다).
    ...
    Object.keys(boundariesByCode).forEach(function (c) {
      ...
      var withFloatNext = occ.filter(function (idx) {
        if (/^\d+\.\d+$/.test(tokens[idx + 1] || '')) return true;
        return /^\d{1,4}$/.test(tokens[idx + 1] || '') && /^\d+\.\d+$/.test(tokens[idx + 2] || '');
      });
    ...
    if (!boundaries.length) {
      ...
      lines(text).forEach(function (line) {
        ...
        var parsed2 = extractAreaFromTokens(lineToks, preferContractArea);
    ...
    for (var b = 0; b < boundaries.length; b++) {
      ...
      // 각주("• ...")가 두 타입의 실제 데이터 행 사이에 끼어드는 문서가 있다(실사례: 전남
      // 광주통합특별시 북구 임동 아파트 - 마지막 두 타입 사이에 "• 평형환산방법 :
      // 공급면적(㎡) × 0.3025 또는 ..." 같은 각주가 통째로 낀다). 이 각주 안 소수값(0.3025,
      // 3.3058)이 세그먼트에 섞이면 "면적값 뒤 첫 정수"를 찾는 세대수 추출 로직이 그
      // 각주 뒤쪽(다음 타입의 관리번호 등 무관한 숫자)까지 밀려 엉뚱한 값을 세대수로
      // 잘못 채택한다. "•"로 시작하는 각주 프로즈는 데이터 행에 나올 수 없는 형태이므로,
      // 세그먼트를 그 지점 이전까지로 잘라 각주가 아예 안 섞이게 한다.
      // 페이지 하단 쪽번호("- 4 -")가 타입의 마지막 데이터 행 바로 뒤, 각주보다도 먼저
      // 붙는 문서도 있다(실사례: 힐스테이트 송파더그리드 오피스텔 - "...17.7875 - 4 -
      // • 상기...". 쪽번호의 "4"가 면적값 뒤 첫 정수로 잘못 채택되어 세대수(12)를 밀어내고
      // 엉뚱하게 그 값(4)이 세대수가 됨). "-"+숫자+"-"라는 쪽번호 특유의 형태는 실제 데이터
      // 행에 나올 수 없으므로, 이 패턴도 각주와 같은 방식으로 그 지점 이전까지 자른다.
      for (var bulletIdx = segStart; bulletIdx < segEnd; bulletIdx++) {
        if (tokens[bulletIdx] === '•') { segEnd = bulletIdx; break; }
        if (tokens[bulletIdx] === '-' && /^\d{1,3}$/.test(tokens[bulletIdx + 1] || '') && tokens[bulletIdx + 2] === '-') {
          segEnd = bulletIdx; break;
        }
      }
      if (segEnd <= segStart) continue;
      var parsed = extractAreaFromTokens(tokens.slice(segStart, segEnd), preferContractArea);
      // 2차 시도(면적먼저형처럼 코드 앞에 전용면적이 오는 경우만): 코드 앞 2칸까지 넓혀 재시도.
      if (!parsed) {
        var segStart2 = Math.max(0, segStart - 2);
        parsed = extractAreaFromTokens(tokens.slice(segStart2, segEnd), preferContractArea);
      }
  ...
  // "103.0590A 주택형은 103A1, 103A2, 103A3로 구분되며 ... 통합하여 접수"처럼, 여러 하위코드가
  // 청약 단계에서는 하나로 묶여 코드 없이 반복되는 가격표를 갖는 문서가 있다(실사례: 전남
  // 광주통합특별시 북구 임동 아파트 - "103.0590A" 긴 코드가 표에 한 번만 나오고, 그 뒤로
  // "103A1"에 해당하는 층별 가격 블록 하나, 이어서 코드 표시가 전혀 없는 동일 구조의 블록이
  // 정확히 하위코드 개수(103A2, 103A3)만큼 더 반복된다). "103.0590A"는 longCodeToShortCandidate로
  // "103A"까지만 좁혀지고 "103A1/103A2/103A3" 중 어느 것도 정확히 가리키지 못해 그대로는 매칭
  // 실패한다 - codeSet 안에서 "숫자+문자 접두부가 같고 끝자리 숫자만 다른" 코드가 2개 이상이면
  // 그 접두부를 "패밀리"로 인정해, 트레일링 숫자 오름차순으로 순서를 매겨둔다.
  function deriveCodeFamilies(codeSet) {
    var byPrefix = {};
    (codeSet || []).forEach(function (c) {
      var m = /^(\d{2,4}[A-Za-z]{1,3})(\d{1,2})$/.exec(c);
      if (!m) return;
      (byPrefix[m[1]] = byPrefix[m[1]] || []).push({ code: c, variant: Number(m[2]) });
    });
    var families = {};
    Object.keys(byPrefix).forEach(function (prefix) {
      var members = byPrefix[prefix];
      if (members.length < 2) return; // 우연히 끝자리가 숫자인 단독 코드와 구분하기 위해 최소 2개 이상만 인정
      members.sort(function (a, b) { return a.variant - b.variant; });
      families[prefix] = members.map(function (m) { return m.code; });
    });
    return families;
  }
  ...
  function scanPriceRowsWithMap(tokens, dataStart, colMap, codeSet, unit_mult) {
    var rows = [];
    var codeFamilies = deriveCodeFamilies(codeSet);
    ...
    var familyQueue = []; // 통합청약 패밀리에서 아직 배정되지 않은 나머지 하위코드
    var familyFirstFloor = null; // 현재 코드에서 처음 나온 층 표기(반복 블록의 시작점 판별용)
    var currentDong = '';
    ...
    // "2~17층 최고 ... / 최저 ..."처럼 이미 층 범위가 있는 행에 "최고"/"최저"
    // (뒤에 "가"가 안 붙는 순한글 라벨이라 서브옵션 라벨과 똑같은 모양)가 이어지는
    // 표가 있다(실사례: 청라 피크원 푸르지오 오피스텔). 이 라벨은 "기본형"/
    // "마이너스옵션"과 달리 동/호(위치)가 아니라 그 층 구간 "안"에서의 가격
    // 구간(최고가/최저가)을 가리키므로, 동/라인 칸이 아니라 층 범위 뒤에 붙여야
    // "1호, 2호 최고"(동/호+가격구간이 뒤섞임)처럼 보이지 않는다. 층 범위 없이
    // 코드 뒤에 "최저가"/"최고가"(가 붙음) 두 행만 있는 기존 케이스(1090번째 줄
    // 부근)는 이 함수를 안 거치므로 영향받지 않는다.
    var TIER_LABEL_RE = /^(최고|최저)$/;
    function pushRow(floorRaw, r) {
      var isTierLabel = r.label && TIER_LABEL_RE.test(r.label);
      var floorText = isTierLabel ? (floorRaw + ' ' + r.label) : floorRaw;
      var labelForDong = isTierLabel ? null : r.label;
      currentCodes.forEach(function (code) {
        rows.push({
          code: code,
          floor: parseFloorDesc(floorText),
          // 세대수 컬럼이 없는 표에서는 서브옵션 라벨(기본형/마이너스옵션 등)을
          // 동/라인 칸에 대신 표시해 두 행을 구분할 수 있게 한다.
          dong: labelForDong ? (currentDong ? currentDong + ' ' + labelForDong : labelForDong) : currentDong,
    ...
    while (i < n) {
      ...
      var resolvedFamilyRest = null;
      // 순수 숫자 코드(예: "104")가 세대수로도 문서 안 다른 곳에서 우연히 등장할 수 있다
      // (실사례: 힐스테이트 송파더그리드 오피스텔 - "42B1 104 최저가..."에서 42B1의
      // 세대수 104가 마침 다른 타입의 실제 코드이기도 함). 이 "104"를 코드로 잘못 해석하면
      // 그 뒤 최저가/최고가 값이 엉뚱하게 "104" 타입으로 흡수되고 42B1은 데이터를 통째로
      // 잃는다. "아직 데이터가 없는 현재 코드 바로 뒤에 숫자 하나 + '최저가'가 이어지는"
      // 형태는 그 숫자가 새 코드가 아니라 현재 코드 자신의 세대수임이 구조적으로 명백하므로,
      // 이 경우엔 codeSet 매칭보다 우선해 새 코드로 취급하지 않는다.
      var looksLikeOwnUnitCount = /^\d+$/.test(tok) && currentCodes.length && !groupHasRows && tokens[i + 1] === '최저가';
      if (codeSet && !looksLikeOwnUnitCount) {
        if (codeSet.indexOf(tok) !== -1) resolvedCode = tok;
          ...
          else if (aliasCandidate && codeFamilies[aliasCandidate]) {
            resolvedCode = codeFamilies[aliasCandidate][0];
            resolvedIsAlias = true;
            resolvedFamilyRest = codeFamilies[aliasCandidate].slice(1);
          }
        }
      ...
      if (resolvedCode) {
        ...
        familyQueue = resolvedFamilyRest || [];
        familyFirstFloor = null;
      ...
      if (isFloorToken(tok) && currentCodes.length) {
        ...
        if (r) {
          // 통합청약 패밀리 처리 중, 층 표기가 처음 값으로 되돌아오면(예: "45~49층"까지 다
          // 채운 뒤 다시 "2~4층"이 나옴) 코드 표시 없는 다음 하위코드의 반복 블록이 시작된
          // 것으로 본다. 동/호 목록이 페이지 경계에서 끊겨 블록 중간에 다시 나오는 경우
          // (동 토큰 자체를 새 블록 신호로 삼으면 오탐 - 실측)와 달리, 층 표기가 "한 바퀴
          // 다 돌고 처음으로 되돌아왔는지"는 블록이 진짜로 끝났는지를 안정적으로 알려준다.
          if (familyQueue.length && familyFirstFloor !== null && floorRaw === familyFirstFloor) {
            currentCodes = [familyQueue.shift()];
            groupHasRows = false;
            familyFirstFloor = null;
          }
          if (familyFirstFloor === null) familyFirstFloor = floorRaw;
          pushRow(floorRaw, r);
  ...
  // 통합청약 패밀리(예: 103A1/103A2/103A3)는 공급대상표에 하위코드별 개별 세대수가 아니라
  // 패밀리 전체 합산값 하나만 나온다(실사례: "223"이 103A1/103A2/103A3 셋을 합친 값인데
  // 표 구조상 첫 하위코드에만 붙어 보이고, 나머지 하위코드는 PDF 재구성 순서상 값이 아예
  // 없거나 엉뚱한 숫자가 붙는다). 반면 가격 섹션은 패밀리별로 반복되는 층별 표에서 각
  // 하위코드의 실제 세대수를 정확히 합산해낼 수 있다(scanPriceRowsWithMap의 통합청약
  // 처리 참고). 이 함수는 그 값을 대조해, 같은 패밀리의 가격 섹션 합산 세대수 총합이 공급
  // 대상표에 실제로 나타나는 값(패밀리 구성원 중 누군가에게 표시된 값) 중 하나와 정확히
  // 일치할 때만(=우연이 아니라는 산술적 증거가 있을 때만) 그 패밀리 구성원 전원의
  // supply_units를 가격 섹션에서 계산한 정확한 값으로 교체한다. 일치하지 않으면 원래 값을
  // 그대로 둔다(검증 안 된 값으로 함부로 덮어쓰지 않음 - 명시적 실패가 조용한 오염보다 안전).
  function reconcileFamilyAreaUnits(areaTypes, priceRows) {
    if (!areaTypes || !areaTypes.length || !priceRows || !priceRows.length) return areaTypes;
    var codes = areaTypes.map(function (t) { return t.code; });
    var families = deriveCodeFamilies(codes);
    if (!Object.keys(families).length) return areaTypes;
    var byCode = {};
    areaTypes.forEach(function (t) { byCode[t.code] = t; });
    var unitsByCode = {};
    priceRows.forEach(function (r) { unitsByCode[r.code] = (unitsByCode[r.code] || 0) + (r.units || 0); });
    Object.keys(families).forEach(function (prefix) {
      var members = families[prefix];
      var total = 0, allHavePriceRows = true;
      members.forEach(function (c) {
        if (!(c in unitsByCode)) allHavePriceRows = false;
        total += unitsByCode[c] || 0;
      });
      if (!allHavePriceRows || total <= 0) return;
      var declaredMatches = members.some(function (c) { return byCode[c] && byCode[c].supply_units === total; });
      if (!declaredMatches) return;
      members.forEach(function (c) { if (byCode[c]) byCode[c].supply_units = unitsByCode[c]; });
    });
    return areaTypes;
  }

  // 세대수 컬럼이 아예 없는 가격표(예: "최고/최저 x 3개 층구간"처럼 층구간별 실제 세대수
  // 배분이 원문 어디에도 없는 구조 - 실사례: 청라 피크원 푸르지오 오피스텔)는
  // readMoneyRun의 fallback으로 모든 행이 units=1이 된다. 이 상태를 그대로 두면 "소계"/
  // "합계" 행의 세대수 합계가 공급대상표의 실제 표기 세대수(예: 176세대)와 크게 어긋나
  // 보인다(예: 합계가 6으로 나옴 - 세대수가 아니라 가격 구간 개수이기 때문). 층구간별
  // 실제 배분은 원문에 전혀 없어 지어낼 수 없으므로, "몇 개의 가격 구간이든 같은 비중으로
  // 본다"는 가장 단순하고 검증 가능한 가정으로 표기 세대수를 균등 배분한다(나머지는 앞쪽
  // 행부터 1씩 더해 합이 정확히 표기 세대수가 되게 함) - 26차에서 사용자가 이미 확인한
  // "최저가/최고가 두 행을 절반씩 나누는" 규칙을 N행으로 일반화한 것이다.
  // 그 코드의 "모든" 행이 fallback 기본값(1)일 때만 적용한다 - 하나라도 원문에서 실제
  // 세대수가 파싱된 행이 섞여 있으면(=이미 진짜 정보가 있다는 뜻) 건드리지 않는다. 이미
  // 행 개수와 표기 세대수가 정확히 같으면(예: 최저가/최고가 두 행에 총 2세대) 재배분할
  // 필요 자체가 없으므로 그대로 둔다.
  function redistributeFallbackUnitsWithArea(areaTypes, priceRowsByCode) {
    if (!areaTypes || !priceRowsByCode) return priceRowsByCode;
    areaTypes.forEach(function (t) {
      var rows = priceRowsByCode[t.code];
      if (!rows || !rows.length || !isFinite(t.supply_units) || t.supply_units <= 0) return;
      if (rows.length === t.supply_units) return;
      var allFallback = rows.every(function (r) { return r.units === 1; });
      if (!allFallback) return;
      var base = Math.floor(t.supply_units / rows.length);
      var remainder = t.supply_units - base * rows.length;
      rows.forEach(function (r, idx) { r.units = base + (idx < remainder ? 1 : 0); });
    });
    return priceRowsByCode;
  }

  // 긴 소수형 관리코드가 줄바꿈 지점에서 마지막 숫자 한 자리(+문자 접미사)만 다음 줄로
  // 잘려나가는 경우가 있다(실사례: "200.476\n8B" -> "200.4768B", "207.980\n7F" -> "207.9807F").
  // 이렇게 쪼개지면 앞부분("200.476")은 코드로도 못 쓰이고("소수점 뒤 4자리" 관례에 못 미침),
  // 코드 인식 자체가 실패해 이 타입의 가격 데이터가 통째로 유실되거나(직전 코드에 currentCodes가
  // 남아있으면) 엉뚱한 타입에 잘못 흡수된다. 소수점 뒤 자릿수가 하나 부족한 토큰 바로 뒤에
  // "숫자+문자" 토큰이 이어지고, 합쳤을 때 LONG_CODE_RE와 정확히 일치할 때만 재결합한다.
  function reglueSplitLongCodes(tokens) {
    var out = [];
    for (var i = 0; i < tokens.length; i++) {
      var cur = tokens[i], next = tokens[i + 1];
      if (next && /^\d{2,3}\.\d{1,3}$/.test(cur) && /^\d{1,2}[A-Za-z]{1,2}$/.test(next)) {
        var combined = cur + next;
        if (LONG_CODE_RE.test(combined)) { out.push(combined); i++; continue; }
      }
      out.push(cur);
    }
    return out;
  ...
  function parsePriceSection(text, codes, baseDate) {
    ...
    var allTokens = reglueSplitLongCodes(text.split(/\s+/).filter(Boolean));
  ...
  // 옵션(시스템 에어컨) 열은 실제로 "에어컨" 관련 품목이 있을 때만 채운다(실사례: 힐스테이트
  // 송파더그리드 오피스텔 - "시스템 에어컨" 항목 자체가 문서에 없는데도, 위 두 함수가 "첫
  // 소제목"이라는 이유만으로 전혀 무관한 품목("현관 컬렉션")의 최저가를 대신 채워 넣고
  // 있었다 - 확장비처럼 "해당 항목이 없으면 0/빈칸"이 맞는 동작이다). 소제목(번호형/단위
  // 표기형 둘 다)이 여러 개로 나뉘는 문서는 "에어컨"이 포함된 소제목을 찾아 그 구간만 쓴다.
  function findSubsectionHeadingBoundaries(text) {
    var boundaries = [];
    var reNum = /^\d\)\s*\S/gm;
    var m;
    while ((m = reNum.exec(text))) {
      var lineEnd = text.indexOf('\n', m.index);
      boundaries.push({ index: m.index, heading: text.slice(m.index, lineEnd < 0 ? text.length : lineEnd) });
    }
    var reUnit = /^.{0,80}[(（[]\s*단위\s*[:：][^)）\]]*[)）\]]\s*$/gm;
    while ((m = reUnit.exec(text))) {
      var heading = m[0];
      // "(단위: ...)" 표기가 품목명과 같은 줄이 아니라 그 앞 줄에 따로 떨어져 있는 문서가
      // 있다(실사례: 힐스테이트 송파더그리드 오피스텔 - "■ 현관 컬렉션 (34A㎡~100B㎡)"
      // 줄 다음에 "(단위 : 원/VAT 포함)"만 별도 줄로 이어짐). 이 경우 "(단위:...)" 줄만
      // 보면 품목명이 전혀 안 잡혀 에어컨 여부를 판단할 수 없으므로, 바로 앞 줄까지
      // 합쳐서 키워드 판별에 쓴다.
      if (m.index > 0) {
        var prevLineStart = text.lastIndexOf('\n', m.index - 2) + 1;
        var prevLine = text.slice(prevLineStart, m.index - 1).trim();
        if (prevLine) heading = prevLine + ' ' + heading;
      }
      boundaries.push({ index: m.index, heading: heading });
    }
    boundaries.sort(function (a, b) { return a.index - b.index; });
    // 같은 줄이 번호형/단위표기형 두 패턴에 동시에 걸리면 같은 인덱스가 중복 등록된다
    // (실사례: "1) 시스템에어컨 (단위 : 원/...)" 처럼 번호와 단위표기를 함께 쓰는 줄) -
    // 다음 경계와의 거리를 잴 때 이 중복이 그대로 있으면 "다음 경계"가 자기 자신의
    // 중복이 되어 구간 길이가 0이 되므로, 인덱스 기준으로 중복을 제거한다.
    var deduped = [];
    boundaries.forEach(function (b) {
      if (!deduped.length || deduped[deduped.length - 1].index !== b.index) deduped.push(b);
    });
    return deduped;
  }

  function restrictToAirconSubsection(text) {
    var boundaries = findSubsectionHeadingBoundaries(text);
    // 소제목이 하나 이하(=구분할 근거가 없음)면 기존 로직(전체 텍스트)에 맡긴다.
    if (boundaries.length < 2) return null;
    var airconIdx = -1;
    for (var i = 0; i < boundaries.length; i++) {
      if (/에어컨/.test(boundaries[i].heading)) { airconIdx = i; break; }
    }
    if (airconIdx < 0) return null;
    var start = boundaries[airconIdx].index;
    var end = airconIdx + 1 < boundaries.length ? boundaries[airconIdx + 1].index : text.length;
  ...
  function parseOptionSection(text, codes) {
    var normalized = fixGluedNumbers(despaceKeywords(String(text || '')));
    var boundaries = findSubsectionHeadingBoundaries(normalized);
    if (boundaries.length >= 2) {
      // 소제목이 여러 개로 나뉘는 문서는 "소제목 자체"에 에어컨이 있을 때만 그 구간을
      // 쓴다. 본문 어딘가(다른 품목의 유의사항 등)에 우연히 "에어컨"이라는 단어가
      // 나오는 것만으로는 이 옵션표가 에어컨에 관한 것이라 볼 근거가 안 된다(실사례:
      // 힐스테이트 송파더그리드 오피스텔 - "식탁 펜던트의 경우 에어컨, 외기에 의한
      // 흔들림이 발생할 수 있으면..."이라는 완전히 무관한 문장 때문에, 전체 텍스트
      // 기준 검색으로는 "현관 컬렉션" 소제목이 에어컨 관련으로 잘못 채택될 뻔했다).
      var restricted = restrictToAirconSubsection(normalized);
      if (!restricted) return {};
      return parseAmountByCodeSection(restricted, codes);
    }
    // 소제목 구분이 없는 단일 옵션표 문서는 표 전체에 "에어컨" 언급이 있을 때만(이미
    // 검증된 실사례들이 전부 이 형태) 기존처럼 전체를 그대로 쓴다. 언급 자체가 없으면
    // 확장비와 달리 아무 품목이나 대신 채우면 안 되므로 빈 결과를 반환한다.
    if (!/에어컨/.test(normalized)) return {};
    return parseAmountByCodeSection(normalized, codes);
  }
  ...
  function extractMeta(text) {
    ...
    if (!open_date) {
      // "최초 분양광고일(2025.07.17.)"처럼 날짜가 키워드 "뒤"(괄호 안)에 오는, openM2와
      // 반대 순서의 표기도 있다(실사례: 청라 피크원 푸르지오 오피스텔 - 정정공고문이라
      // "공고일:" 라벨도, "날짜)...승인" 순서도 전혀 없고 "최초 분양광고일(날짜)"만
      // 문서 곳곳에 반복된다).
      var openM3 = text.match(/(?:최초\s*)?분양\s*광고\s*일\s*[(（]\s*(\d{4}[.\-/]\s*\d{1,2}[.\-/]\s*\d{1,2})/);
      if (openM3) open_date = parseFlexDate(openM3[1]);
    }
  ...
  // ---------------------------------------------------------------------
  // ⑤ 중도금 대출 안내 - 타입별 무이자/이자후불제 구분
  //
  // 대부분의 문서는 중도금 무이자/이자후불제 여부가 단지 전체에 하나로 적용돼 사용자가
  // 폼에서 직접 고르면 충분하지만("이자 후불제"/"무이자" 드롭다운), 가끔 타입 구간별로
  // 조건이 다른 문서가 있다(실사례: 힐스테이트 송파더그리드 오피스텔 - "34A㎡, 34B㎡,
  // 36㎡ 타입은 '중도금 무이자 대출', 42㎡~119㎡ 타입은 '중도금 대출 이자 후불제'").
  // "■ 중도금 대출 안내" 섹션 안에서 "(코드 목록 또는 면적 범위) 타입은 ...(무이자|이자
  // 후불제)..." 형태의 절을 찾아 코드별 조건을 돌려준다. 이 섹션 자체가 없거나 타입
  // 구분 없이 하나로만 쓰여 있는 문서(대다수)는 빈 객체를 반환 - 이 경우 UI가 그대로
  // 단지 전체 설정(드롭다운)을 쓰므로 회귀 없음.
  // ---------------------------------------------------------------------
  var INTEREST_HEADING_RE = /[■▣]\s*중도금\s*대출\s*안내/;

  function resolveInterestCodeSpec(spec, codes) {
    // 범위형: "42㎡~119㎡" 또는 "42~119" - 문서 원문의 면적 숫자 접두부 범위로, 실제
    // 타입 코드(codes)의 숫자 접두부가 그 범위 안에 들면 전부 포함한다.
    var rangeM = /^(\d+(?:\.\d+)?)\s*㎡?\s*~\s*(\d+(?:\.\d+)?)\s*㎡?$/.exec(spec);
    if (rangeM) {
      var lo = Number(rangeM[1]), hi = Number(rangeM[2]);
      return (codes || []).filter(function (c) {
        var n = Number((String(c).match(/^\d+/) || [])[0]);
        return isFinite(n) && n >= lo && n <= hi;
      });
    }
    // 목록형: "34A㎡, 34B㎡, 36㎡" - ㎡ 단위 표기를 떼고 실제 코드와 대조한다.
    var parts = spec.split(',').map(function (s) { return s.replace(/㎡/g, '').trim(); }).filter(Boolean);
    return parts.filter(function (p) { return !codes || codes.indexOf(p) !== -1; });
  }

  function parseInterestSection(text, codes) {
    var result = {};
    text = String(text || '');
    var m = INTEREST_HEADING_RE.exec(text);
    if (!m) return result;
    var rest = text.slice(m.index + m[0].length);
    var nextHeadingIdx = rest.search(/[■▣]/);
    var body = nextHeadingIdx >= 0 ? rest.slice(0, nextHeadingIdx) : rest;

    // "(코드 목록/범위) 타입은 ...무이자..." 또는 "...이자 후불제..." - 코드 목록과
    // 키워드 사이에 마침표(다음 문장)가 끼면 안 되므로 [^.]로 문장 경계를 넘지 않는다.
    var clauseRe = /([0-9A-Za-z㎡,\s~.]+?)\s*(?:타입|형)\s*은\s*[^.]*?(무이자|이자\s*후불제|후불제)/g;
    var cm;
    while ((cm = clauseRe.exec(body))) {
      var codeSpec = cm[1].replace(/^[,\s]+|[,\s]+$/g, '');
      var kind = /무이자/.test(cm[2]) ? '무이자' : '후불제';
      resolveInterestCodeSpec(codeSpec, codes).forEach(function (c) { result[c] = kind; });
    }
    return result;
  }
  ...
  var SECTION_ANCHORS = {
    ...
    // 대분류 제목("Ⅰ 공급내역 및 공급금액")과 표 제목이 분리된 문서는 표 제목 자체가
    // 불릿도 "및 면적"도 없이 그냥 "공급대상 (단위: ㎡, 호실)"처럼 단위표기 괄호가 바로
    // 뒤에 붙는 경우가 있다(실사례: 청라 피크원 푸르지오 오피스텔). "공급대상물"(전혀
    // 다른 절 - 대지면적/연면적 등 사업개요)과 헷갈리지 않도록 "물"이 바로 뒤에 오면
    // 제외한다.
    area: [/[■▣]\s*공급대상\s*(?:및\s*공급규모)?(?!물)/, /^\s*공급대상\s*및\s*면적/m, /^\s*공급대상(?!물)\s*[(（]/m, /[■▣]\s*공급면적\s*(?:및\s*공급규모)?/],
    ...
    // area와 마찬가지로 "공급금액 및 납부일정 (단위: 원, VAT포함)"처럼 불릿 없이 표
    // 제목만 줄 시작에 오는 문서가 있다(실사례: 청라 피크원 푸르지오 오피스텔 - 기존
    // 불릿 없는 폴백은 "공급대금"이라 다른 단어라 못 걸렸다).
    price: [/[■▣]\s*공급금액\s*및\s*납부\s*일정/, /[■▣]\s*분양가격\s*납부조건\s*등?\s*안내/, /[■▣]\s*공급금액\s*납부조건\s*등?\s*안내/, /[■▣]\s*공급금액\s*표(?![가-힣])/, /^\s*공급대금\s*및\s*납부일정/m, /^\s*공급금액\s*및\s*납부\s*일정/m],
    balcony: [/[■▣]\s*발코니\s*확장/],
    ...
    // "■ 선택품목(유상옵션) 공급금액 및 납부일정"처럼 "추가"로 시작하지 않고 "선택품목
    // (유상옵션)"으로 시작하는 헤딩도 있다(실사례: 청라 피크원 푸르지오 오피스텔).
    option: [/[■▣]\s*추가[\s가-힣]{0,6}옵션품목/, /[■▣]\s*옵션품목/, /[■▣]\s*별도계약\s*[-–]\s*추가\s*선택품목/, /[■▣]\s*추가\s*선택품목/, /[■▣]\s*선택품목\s*[(（]\s*유상\s*옵션\s*[)）]/]
  };
  ...
  function splitDocumentSections(fullText) {
    ...
    if (areaStart >= 0 && priceStart >= 0) {
      ...
      // 두 번째(별도) 가격표가 문서 뒤쪽에, balcony/option 앵커보다도 늦게 또 있는 경우가
      // 있다(실사례: 전남광주통합특별시 북구 임동 아파트 - "최상층 특화(펜트) 세대"만
      // 따로 다루는 두 번째 표가 "■ 공통 유의사항" 섹션 안에 흩어져 있다. 힐스테이트
      // 송파더그리드 오피스텔은 아예 "■" 헤딩 없이 긴 유의사항 프로즈 뒤에 표 헤더만
      // 다시 찍고 곧장 이어짐). 이 두 번째 표는 다른 타입(200B/207F/209C/211E 등)의
      // 가격을 메인 표와 중복해서 다시 보여주기도 해서, 통째로 이어붙이면 그 타입들의
      // 세대수가 두 배로 잘못 집계된다. 그래서 표 전체를 붙이는 대신, 기존 복구를 다
      // 거치고도 여전히 가격 행이 하나도 없는 코드만 targeted로 찾는다.
      //   - 긴 소수형 코드는 줄바꿈 지점에서 쪼개진 채("214.618\n3A ...") 남아 있어, 줄
      //     시작에 층 표기가 와야 하는 repairUndercountedPriceRows의 재개 지점 판별로도
      //     못 찾는다(그 줄이 "3A 107동 1호 1 49층 1 ..."처럼 코드 조각으로 시작) - area
      //     섹션에서 이미 읽어낸 전용면적으로 온전한 긴 코드 문자열을 재구성해(예: "214A"
      //     + 214.6183 -> "214.6183A") 찾는다.
      //   - 층 표기 자체가 아예 없는 "코드+최저가/최고가" 형식(순수 숫자 코드 등)은 긴
      //     코드 재구성이 필요 없이 코드 자신이 그대로 다시 등장하므로, 코드 문자열
      //     그대로 찾는다.
      // 두 경우 다, 그 코드가 실제로 다시 등장하지 않으면 아무 것도 붙이지 않으므로 다른
      // 문서에는 영향이 없다. 탐색 시작점도 "■" 헤딩이 있으면 그 지점, 없으면 기존 가격
      // 섹션이 끝난 지점(priceEndIdx)부터 최대한 넓게(발코니/옵션 경계 또는 문서 끝까지)
      // 잡는다 - 헤딩 유무와 무관하게 targeted 검색이므로 범위를 넓혀도 안전하다.
      var beforeExtra = parsePriceSection(price, areaCodes).priceRows;
      var seenBeforeExtra = {};
      beforeExtra.forEach(function (r) { seenBeforeExtra[r.code] = true; });
      var stillMissingCodes = areaCodes.filter(function (c) { return !seenBeforeExtra[c]; });
      var extraPriceStart = -1;
      if (stillMissingCodes.length && priceEndIdx >= 0) {
        var extraHeadingStart = findEarliestMatch(text, SECTION_ANCHORS.price, priceEndIdx);
        extraPriceStart = extraHeadingStart >= 0 ? extraHeadingStart : priceEndIdx;
      }
      if (extraPriceStart >= 0) {
        var widerPriceBound = firstPositive(balconyStart > extraPriceStart ? balconyStart : -1,
          optionStart > extraPriceStart ? optionStart : -1, nextMajorStart > extraPriceStart ? nextMajorStart : -1);
        if (widerPriceBound < 0) widerPriceBound = text.length;
        var extraTokens = reglueSplitLongCodes(text.slice(extraPriceStart, widerPriceBound).split(/\s+/).filter(Boolean));
        var areaByCode = {};
        areaTypesForRepair.forEach(function (a) { areaByCode[a.code] = a; });
        var snippets = [];
        stillMissingCodes.forEach(function (code) {
          var a = areaByCode[code];
          if (!a) return;
          var letterSuffix = (code.match(/[A-Za-z]+\d*$/) || [''])[0];
          // "■ 주택형 표시 안내" 같은 코드 매핑 표에도 같은 코드가 먼저 나올 수 있어,
          // 첫 번째 occurrence가 아니라 마지막 occurrence를 쓴다 - 실제 가격 행은 항상
          // 이 문서에서 더 뒤쪽에 있다.
          var idx = -1;
          if (letterSuffix && isFinite(a.exclusive_area)) {
            idx = extraTokens.lastIndexOf(a.exclusive_area.toFixed(4) + letterSuffix);
          }
          if (idx === -1) idx = extraTokens.lastIndexOf(code);
          if (idx === -1) return;
          // 코드+세대수 뒤에 "최저가"/"최고가" 두 행(각 라벨+금액 12개 = 13토큰)이 이어지는
          // 형식은 한 쌍을 통째로 담으려면 2+13+13=28토큰이 필요하다 - 너무 좁게 자르면
          // 최고가 쪽 금액이 중간에 잘려 파싱 자체가 실패한다(실측). 넉넉히 36토큰까지 담되,
          // 그 사이에 다른 area 코드가 먼저 등장하면(자기 행이 그보다 짧다는 뜻) 거기서
          // 끊는다 - 안 그러면 뒤이은 코드의 행 일부가 이 스니펫에 얹혀 따라와, 나중에 그
          // 코드 자신의 스니펫과 합쳐질 때 같은 행이 두 번 잡혀 세대수가 부풀려진다(실측:
          // 118의 최고가 행이 115 스니펫에 얹혀와 118 스니펫과 중복).
          var windowEnd = Math.min(extraTokens.length, idx + 36);
          for (var wi = idx + 2; wi < windowEnd; wi++) {
            if (areaCodes.indexOf(extraTokens[wi]) !== -1) { windowEnd = wi; break; }
          }
          snippets.push(extraTokens.slice(idx, windowEnd).join(' '));
        });
        if (snippets.length) {
          var mergedWithExtra = (price + '\n' + snippets.join('\n')).trim();
          var afterExtra = parsePriceSection(mergedWithExtra, areaCodes).priceRows;
          if (afterExtra.length > beforeExtra.length) price = mergedWithExtra;
        }
      }
  ...
  var api = {
    ...
    reconcileFamilyAreaUnits: reconcileFamilyAreaUnits,
    redistributeFallbackUnitsWithArea: redistributeFallbackUnitsWithArea,
    parseBalconySection: parseBalconySection,
    parseOptionSection: parseOptionSection,
    extractMeta: extractMeta,
    parseInterestSection: parseInterestSection,
