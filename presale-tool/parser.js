    ...
    while (i < n) {
      ...
      if (codeSet) {
        if (codeSet.indexOf(tok) !== -1) resolvedCode = tok;
          ...
          if (aliasCandidate && codeSet.indexOf(aliasCandidate) !== -1) resolvedCode = aliasCandidate;
        }
      } else if (SHORT_CODE_RE.test(tok) || LONG_CODE_RE.test(tok)) {
        resolvedCode = tok;
      }
      if (resolvedCode) { currentCode = resolvedCode; currentDong = ''; currentFloorRaw = null; lastDongIdx = -1; i++; continue; }
