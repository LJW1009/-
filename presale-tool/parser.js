...
(function (root) {
  ...
  // "최저가"/"최고가" 두 행의 값을 항목별로 평균해 하나로 합친다. 분할 방식(대지비/건축비/
  // 부가세/계약금/중도금 회차/잔금)이 두 값 모두 같은 비율 구조이므로, 항목별 평균의 합이
  // 총액(price) 평균과도 일치해 내적 일관성이 그대로 유지된다.
  function averagePriceValues(a, b) {
    var out = {};
    ['total', 'down', 'land', 'build', 'vat', 'balance'].forEach(function (k) {
      if (a[k] != null && b[k] != null) out[k] = Math.round((a[k] + b[k]) / 2);
      else out[k] = a[k] != null ? a[k] : b[k];
    });
    out.mid = (a.mid || []).map(function (v, idx) {
      var bv = (b.mid || [])[idx];
      return bv != null ? Math.round((v + bv) / 2) : v;
    });
    return out;
  }
  ...
  function scanPriceRowsWithMap(tokens, dataStart, colMap, codeSet, unit_mult) {
    ...
    while (i < n) {
      ...
      // 사이의 실제 세대수 분배는 문서 어디에도 나와 있지 않으므로 지어내지 않고, 두 값을
      // 평균한 값 하나로 합쳐 전체 세대수(코드 뒤 숫자)를 그대로 쓴다(합계가 실사례와 어긋나지
      // 않도록 - 세대수를 나눠 배정하면 검증 불가능한 값을 지어내는 셈이 된다).
      if (/^\d+$/.test(tok) && currentCodes.length && !groupHasRows && tokens[i + 1] === '최저가') {
        ...
        if (lowRun && lowRun.label === '최저가' && tokens[lowRun.nextIndex] === '최고가') {
          ...
          if (highRun && highRun.label === '최고가') {
            var avgValues = averagePriceValues(lowRun.values, highRun.values);
            currentCodes.forEach(function (code) {
              rows.push({
                code: code,
                floor: parseFloorDesc('전체'),
                dong: currentDong,
                units: typeUnits,
                price: avgValues.total,
                down_payment: avgValues.down != null ? avgValues.down : null,
                down_is_ratio: false,
                down_ratio: null,
                land: avgValues.land,
                build: avgValues.build,
                vat: avgValues.vat,
                balance: avgValues.balance,
                mid_amounts: avgValues.mid
              });
            });
            groupHasRows = true;
            i = highRun.nextIndex;
