...
(function (root) {
  ...
  function detectPriceColumnStructure(tokens) {
    ...
    return { offsetCount: Number(parts[0]), totalColumnCount: Number(parts[1]) };
  }
  ...
  // "N + 한글단어"(1안/2안/4대 등) 패턴을 일반화해 특정 단어 목록에 덜 의존하게 하고,
  // 흔한 라벨(기본/전실/전체/추가)은 번호 없이도 인식한다.
  var CONTINUATION_RE = /^(\d+\s*[가-힣]+|기본|전실|전체|추가)/;
