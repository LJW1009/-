...
async function main() {
  ...
  // 29차 후속7: 금액 열을 전부 천원 단위로 통일하면서, 이자 계산식에 박히는 원금도
  // 1000으로 나눈 값(383600, 95900)을 쓴다(그래야 결과인 이자 금액도 다른 금액 열과
  // 같은 천원 단위가 된다).
  check('M07', '1차 중도금 이자 수식이 실제 원금(383,600,000원 → 천원 단위 383600, 분양가의 10% 가정이 아님)과 자기 날짜(W$7)를 정확히 참조',
    beFormula === '(383600*$V$6)*(W$7/365)');
  check('M08', '2차 중도금 이자 수식이 실제 원금(95,900,000원 → 천원 단위 95900)과 자기 날짜(X$7)를 정확히 참조(더 이상 한 칸씩 밀리지 않음)',
    bfFormula === '(95900*$V$6)*(X$7/365)');
