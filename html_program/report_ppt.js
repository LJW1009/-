...
/* PPT_T (색상·폰트 토큰)는 ppt.js에서 공용으로 정의됨 */
...
function buildReportPpt(spec) {
  ...
  function heading(slide, text, y) {
    slide.addText(text, { x: RPT_M.x, y: y, w: RPT_M.w, h: RPT_M.headH, fontFace: PPT_T.FONT, fontSize: 14, bold: true, color: PPT_T.TEXT, margin: 0, valign: "middle" });
  ...
  function cover(slide) {
    ...
    if (spec.meta.subtitle) {
      slide.addText(spec.meta.subtitle, { x: 0.601, y: 1.94, w: 9.63, h: 0.32, fontFace: PPT_T.FONT, fontSize: 12, bold: true, color: "595959", align: "center", valign: "middle", margin: 0 });
    }
    ...
    slide.addText(
      ...
      { x: 2.964, y: 5.7, w: 4.497, h: 1.043, fontFace: PPT_T.FONT, fontSize: 28, bold: true, color: PPT_T.TEXT, align: "center", valign: "top" }
    );
    slide.addText(spec.meta.source, { x: 0, y: 7.087, w: 8.6, h: 0.3, fontFace: PPT_T.FONT, fontSize: 12, bold: true, color: PPT_T.TEXT, valign: "middle" });
  }
  ...
  function renderBox(slide, b, y) {
    ...
    const h = n * LH + 0.3;
    slide.addShape(pres.ShapeType.rect, { x: RPT_M.x - 0.067, y: y, w: RPT_M.w, h: h, fill: { color: PPT_T.BOX }, line: { color: PPT_T.BOX, width: 0.5 } });
    const body = [];
    b.items.forEach((t, i) => {
    ...
    slide.addText(body, { x: RPT_M.x, y: y - 0.106, w: RPT_M.w, h: h + 0.135, fontFace: PPT_T.FONT, fontSize: PT, bold: true, color: PPT_T.TEXT, lineSpacingMultiple: 1.5, valign: "top", margin: [8, 8, 2, 2] });
  ...
  function renderTable(slide, b, y, bottomLimit) {
    ...
    const head = b.columns.map((c) => ({ text: c, options: { fill: { color: PPT_T.TBL_HEAD }, color: "FFFFFF", bold: true, align: "center", valign: "middle" } }));
    const body = b.rows.map((r) => r.map((cell, ci) => ({ text: pptRuns(cell, { bold: true, color: PPT_T.TEXT }), options: { align: ci === 0 ? "center" : "left", valign: "middle" } })));
    ...
    slide.addTable([head].concat(body), { x: RPT_M.x, y: y, w: RPT_M.w, colW: cw, rowH: [0.26].concat(rowHs), border: { type: "solid", color: PPT_T.LINE, pt: 0.75 }, fontFace: PPT_T.FONT, fontSize: pt, margin: [2, 7, 2, 7], autoPage: false });
  ...
  function renderInfo(slide, b, y, bottomLimit) {
    ...
    [{ d: b.before, x: RPT_M.x, fill: "F0F0F0", fg: PPT_T.TEXT, sub: "595959" }, { d: b.after, x: RPT_M.x + CW + AW + 0.24, fill: "1F3864", fg: "FFFFFF", sub: "D6DCE8" }].forEach((s) => {
      if (!s.d) return;
      ...
      slide.addText(s.d.value, { x: s.x, y: y + 0.36, w: CW, h: Math.max(0.36, upperH - 0.92), fontFace: PPT_T.FONT, fontSize: s.d.valueSize || 19, bold: true, color: s.fg, align: "center", valign: "middle", margin: 0 });
    ...
    if (cards.length) {
      ...
      cards.forEach((c, i) => {
        ...
        slide.addText((c.lines || []).map((t, k, a) => ({ text: t, options: { breakLine: k !== a.length - 1 } })), { x: cx + 0.04, y: ly + 0.76, w: colW - 0.08, h: lowerH - 0.84, fontFace: PPT_T.FONT, fontSize: 9.5, bold: true, color: PPT_T.TEXT, align: "center", valign: "top", margin: 0, lineSpacingMultiple: 1.12 });
      });
    }
  ...
  function renderItemHead(slide, b, y) {
    ...
    slide.addText(String(b.no), { x: RPT_M.x + 0.07, y: y + 0.035, w: 0.27, h: 0.27, fontFace: PPT_T.FONT, fontSize: 10, bold: true, color: "FFFFFF", align: "center", valign: "middle", margin: 0 });
    const runsArr = [];
    if (b.tag) runsArr.push({ text: "[" + b.tag + "] ", options: { color: PPT_T.BLUE } });
    runsArr.push.apply(runsArr, pptRuns(b.title, { color: "1F3864" }));
    slide.addText(runsArr, { x: RPT_M.x + 0.44, y: y, w: RPT_M.w - 0.5, h: H, fontFace: PPT_T.FONT, fontSize: 11.5, bold: true, color: "1F3864", align: "left", valign: "middle", margin: 0 });
    return y + H + 0.08;
  }
  ...
  function renderCompare(slide, b, y) {
    ...
    [{ d: b.left, x: RPT_M.x, fill: "F0F0F0", fg: PPT_T.TEXT, sub: "595959" }, { d: b.right, x: RPT_M.x + CW + AW + 0.22, fill: "1F3864", fg: "FFFFFF", sub: "D6DCE8" }].forEach((s) => {
      slide.addShape(pres.ShapeType.roundRect, { x: s.x, y: y, w: CW, h: H, rectRadius: 0.07, fill: { color: s.fill }, line: { color: s.fill, width: 0.5 } });
      slide.addText(s.d.label, { x: s.x, y: y + 0.05, w: CW, h: 0.19, fontFace: PPT_T.FONT, fontSize: 9.5, bold: true, color: s.sub, align: "center", valign: "middle", margin: 0 });
      slide.addText(pptRuns(s.d.value, { color: s.fg }), { x: s.x + 0.08, y: y + 0.25, w: CW - 0.16, h: H - 0.3, fontFace: PPT_T.FONT, fontSize: s.d.size || 12.5, bold: true, color: s.fg, align: "center", valign: "middle", margin: 0 });
    });
  ...
  function renderNote(slide, b, y) {
    ...
    slide.addText(pptRuns(b.text, { bold: false, color: "595959" }), { x: RPT_M.x + 0.02, y: y, w: RPT_M.w, h: h, fontFace: PPT_T.FONT, fontSize: PT, color: "595959", align: "left", valign: "top", margin: 0 });
    return y + h + 0.1;
  }

  function renderBar(slide, lines) {
    const PT = 11, LH = (PT * 1.35) / 72;
    const n = lines.reduce((a, t) => a + pptEstLines("- " + t, PPT_T.W - 0.9, PT, 0.1), 0);
    const h = Math.max(0.68, n * LH + 0.22);
    const y = PPT_T.H - h - 0.06;
    slide.addShape(pres.ShapeType.rect, { x: 0, y: y, w: PPT_T.W, h: h, fill: { color: PPT_T.BAR }, line: { color: PPT_T.BAR, width: 0.5 } });
    const rr = [];
    lines.forEach((t, i) => {
    ...
    slide.addText(rr, { x: 0.25, y: y, w: PPT_T.W - 0.5, h: h, fontFace: PPT_T.FONT, fontSize: PT, bold: true, color: PPT_T.TEXT, align: "left", valign: "middle", margin: 0 });
    return y;
  }
  ...
  (spec.slides || []).forEach((sl) => {
    ...
    const bottom = bar ? renderBar(slide, bar.lines) - 0.14 : PPT_T.H - 0.22;
